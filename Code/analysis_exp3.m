...
for isub = 1:numel(files)
    ...
    data(abs(error)>guessError(exisingSub+isub),:) = [];
    target = data(:,17);
...
% anova
data = [fogError1(:,1), remError(:,1), fogError1(:,2), remError(:,2)];
...
saveas(gca,fullfile(mainPath,'results/exp3', 'neutral_item.svg'));
saveas(gca,fullfile(mainPath,'results/exp3', 'neutral_item.png'));
...
saveas(gca,fullfile(mainPath,'results/exp3', 'cued_item.svg'));
saveas(gca,fullfile(mainPath,'results/exp3', 'cued_item.png'));
%% helper functions
